...
function printHelp() {
  echo "Usage: "
  echo "  start.sh <mode> [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>] [-l <language>] [-i <imagetag>] [-v]"
  ...
  echo "        start.sh up -c mychannel -s couchdb -i 1.2.x"
...
# 检查docker images 和 fabric tools的版本
...
# 判断本地当前版本是否在不支持版本系列里 echo "1.3.0" |grep -q "1.3.0"
...
function replacePrivateKey() {
  ...
  cp extends/docker-compose-e2e-template.yaml extends/docker-compose-e2e.yaml
  ...
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" extends/docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/org2.example.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA2_PRIVATE_KEY/${PRIV_KEY}/g" extends/docker-compose-e2e.yaml
...
#
COMPOSE_FILE_COUCH=extends/docker-compose-couch.yaml
# org3 docker compose file
COMPOSE_FILE_ORG3=extends/docker-compose-org3.yaml
#
